...
typedef std::function<void (const ParallelWork *)> parallel_callback_t;
...
public:
	 ...
	 * But the callback of this canceled series will be still called. */
	void cancel() { this->canceled = 1; }
...
public:
	/* Called only in SubTask::done(). Not for common use. */
	SubTask *pop();

private:
	void set_last_task(SubTask *last)
	{
		last->set_series(this);
		this->last = last;
	}
