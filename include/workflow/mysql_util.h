...
// length of buffer needed to store this number [1, 3, 4, 9].
static inline unsigned int get_field_length_size(const char *pos)
{
	if (*pos <= 251)
		return 1;
...
unsigned long long decode_length(const char **position);

// reserve for server
char *encode_length(char *position, unsigned long long length);
