...
using web_handler_t = std::function<void (const HttpRequest&, HttpResponse&)>;
...
public:
	static int get_peer_addr(const HttpRequest& req, struct sockaddr *addr, socklen_t *addrlen);
	static WFHttpTask *task_of(const HttpRequest& req);
	static WFHttpTask *task_of(const HttpResponse& resp);

private:
	WFWebProcessor processor_;
	static size_t http_req_offset;
	static size_t http_resp_offset;
};
