...
{
	PACKET_HEADER_OK	=	0,
	PACKET_HEADER_NULL	= 251, //0xfb
	PACKET_HEADER_EOF	= 254, //0xfe
	PACKET_HEADER_ERROR	= 255, //0xff
};
...
typedef struct __mysql_parser
{
	...
	size_t err_msg_offset; 			// NULL_LENGTH for default
	int err_msg_len; // NULL_LENGTH for default
...
static inline void mysql_parser_get_info(const char **info_str,
										 size_t *info_len,
...
static inline void mysql_parser_get_net_state(const char **net_state_str,
											  size_t *net_state_len,
											  mysql_parser_t *parser)
{
	*net_state_str = (const char *)parser->buf + parser->net_state_offset;
	*net_state_len = SQLSTATE_LENGTH;
}

static inline void mysql_parser_get_err_msg(const char **err_msg_str,
											size_t *err_msg_len,
											mysql_parser_t *parser)
{
	if (parser->err_msg_offset == NULL_LENGTH && parser->err_msg_len == 0)
	{
		*err_msg_str = SQLSTATE_DEFAULT;
		*err_msg_len = SQLSTATE_LENGTH;
	} else {
