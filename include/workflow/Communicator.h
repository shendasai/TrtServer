...
{
public:
	void set_duration(unsigned int duration) { this->duration = duration; }

private:
	virtual void handle(int state, int error) = 0;

protected:
	unsigned int get_duration() { return this->duration; }

private:
	unsigned int duration;

public:
	SleepSession() { this->duration = 0; }
...
public:
	...
	int drain(int max, CommService *service);
...
private:
	...
	static int awake(const void *buf, size_t *size, poller_message_t *msg);

	static poller_message_t timer_msg;
	...
	static void *aio_event(void *context);
#endif
